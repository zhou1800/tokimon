...
AGENT_WORKTREE_RULE_LINES: list[str] = [
    ...
    "- If your runtime can execute git/shell commands, do not wait for extra human instructions: create a nested worktree before edits, commit any selected improvement you intend to keep, merge verified committed work back into the session checkout, then delete the nested worktree once it is safely merged and clean.",
    ...
    "- Allowed mutating git commands for this rule are only the commands used in the `Create`, `Commit`, `Merge`, and `Delete` sequences below.",
    ...
    *,
    ...
    parts = [
        ...
        *AGENT_WORKTREE_RULE_LINES,
        ...
        "9) If you select an improvement to keep and git/shell is available, commit it in the nested worktree before final verification or merge.",
        "10) Verify final outcome; if verification fails, return details so the orchestrator retries from step 3.",
        "",
